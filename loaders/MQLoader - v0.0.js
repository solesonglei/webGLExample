...
THREE.MQLoader.prototype = {
	...
	{		
		...
		for (var i = 0 ; i < modelNum; ++i)
		{
			...
			all_vertices = new Float32Array(facesNum * 3 * 3);
			vertices     = new Float32Array(verticesNum * 3);
			colors       = new Uint8Array(facesNum * 3 * 3 );
			...
			for (var j = 0; j < facesNum;  ++j){
				...
				all_vertices[j * 9]     = vertices[a * 3];
				all_vertices[j * 9 + 1] = vertices[a * 3 + 1];
				all_vertices[j * 9 + 2] = vertices[a * 3 + 2];
				
				all_vertices[j * 9 + 3] = vertices[b * 3];
				all_vertices[j * 9 + 4] = vertices[b * 3 + 1];
				all_vertices[j * 9 + 5] = vertices[b * 3 + 2];
				
				all_vertices[j * 9 + 6] = vertices[c * 3];
				all_vertices[j * 9 + 7] = vertices[c * 3 + 1];
				all_vertices[j * 9 + 8] = vertices[c * 3 + 2];
			}
			...
			for(var j = 0; j < facesNum * 3 * 3; j += 3){
			...
			//console.log(modelName, all_vertices.length);
			geometry.addAttribute( 'position', new THREE.BufferAttribute( all_vertices, 3 ) );
