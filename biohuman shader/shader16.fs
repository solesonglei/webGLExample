...
varying vec2  SCENEJS_vUVCoord0;
uniform sampler2D SCENEJS_uSampler0;
uniform float SCENEJS_uLayer0BlendFactor;
uniform sampler2D SCENEJS_uSampler1;
uniform float SCENEJS_uLayer1BlendFactor;
...
vec3 worldNormal = normalize(SCENEJS_vWorldNormal); 
  ...
  vec3    viewNormalVec = vec3(0.0, 1.0, 0.0);
...
texturePos = vec4(SCENEJS_vUVCoord0.s, SCENEJS_vUVCoord0.t, 1.0, 1.0);
textureCoord=texturePos.xy;
color = ((1.0 - SCENEJS_uLayer0BlendFactor) * color) + (SCENEJS_uLayer0BlendFactor * texture2D(SCENEJS_uSampler0, vec2(textureCoord.x, 1.0 - textureCoord.y)).rgb);
texturePos = vec4(SCENEJS_vUVCoord0.s, SCENEJS_vUVCoord0.t, 1.0, 1.0);
textureCoord=texturePos.xy;
viewNormalVec = normalize(texture2D(SCENEJS_uSampler1, vec2(textureCoord.x, -textureCoord.y)).xyz * 2.0 - 1.0);
  vec4    fragColor;
  ...
  float   spotDirRatio;
  float   lightDist;
vec3 tangent = normalize(SCENEJS_vTangent);
vec3 bitangent = cross(SCENEJS_vViewNormal, tangent);
mat3 TBM = mat3(tangent, bitangent, SCENEJS_vViewNormal);
